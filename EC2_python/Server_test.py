...
def handle_receive_status(client_socket):
    while 1:
        # try:
        f_size = client_socket.recv(1024)
        f_size = f_size.decode()
...
def accept_func():
    ...
    while 1:
        try:
            ...
            client_socket_st2, addr6 = server_socket_st2.accept()
            print("접속 완료")
        ...
        st2_thread = threading.Thread(target=handle_receive_status, args=(client_socket_st2,))
        send_ras2_thread.daemon = True
        rcv2_thread.daemon = True
        st2_thread.daemon = True
